...
import 'package:boszhan_trading/pages/incoming/incoming_edit_page.dart';
import 'package:boszhan_trading/pages/incoming/incoming_order_history_products.dart';
...
class IncomingHistoryPage extends StatefulWidget {
  const IncomingHistoryPage({Key? key}) : super(key: key);

  @override
  State<IncomingHistoryPage> createState() => _IncomingHistoryPageState();
}

class _IncomingHistoryPageState extends State<IncomingHistoryPage> {
  List<IncomingOrderHistoryModel> orders = [];
            ...
            child: Column(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Column(
                    children: [
                      const Text("Журнал поступления товара",
            ...
            IconButton(
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (context) =>
                          IncomingOrderHistoryProductsPage(order: orders[i])),
                );
              },
              ...
              onPressed: () {
                ...
                if (formattedOrdersDate == formattedDate) {
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                        builder: (context) =>
                            IncomingEditPage(order: orders[i])),
                  );
                } else {
  ...
  void getHistory() async {
    try {
      var response =
          await MainApiService().getIncomingOrderHistory(dateFrom, dateTo);

      orders = [];

      for (var item in response) {
        orders.add(IncomingOrderHistoryModel.fromJson(item));
      }
  ...
  void _showPasswordEntering(IncomingOrderHistoryModel order) async {
